'use client'

import React, { createContext, useContext, useState, useEffect } from 'react'

type Language = 'es' | 'en'

type Translations = Record<Language, Record<string, string>>

const translations: Translations = {
       es: {
              'dashboard': 'Panel',
              'search_placeholder': 'Buscar algo...',
              'new_client': 'Nuevo Cliente',
              'quick_sale': 'Venta Rápida',
              'reports': 'Reportes',
              'tournaments': 'Torneos y Ligas',
              'tournaments_desc': 'Gestiona campeonatos, zonas, fixtures y resultados.',
              'create_tournament': 'Crear Torneo',
              'no_tournaments': 'No tienes torneos activos',
              'no_tournaments_desc': 'Crea tu primer torneo para empezar a gestionar inscripciones y partidos.',
              'draft': 'Borrador',
              'active': 'En Curso',
              'completed': 'Finalizado',
              'manage_tournament': 'Gestionar Torneo',
              'categories': 'Categorías',
              'teams': 'Equipos',
              'matches': 'Partidos',
              'revenue_by_category': 'Ingresos por Categoría',
              'occupancy_by_court': 'Ocupación por Cancha',
              'total_income': 'Ingresos Totales',
              'avg_occupancy': 'Ocupación Media',
              'avg_ticket': 'Ticket Promedio',
              'new_clients': 'Nuevos Clientes',
              'export': 'Exportar',
              'day': 'Hoy',
              'week': 'Semana',
              'month': 'Mes',
              'year': 'Año',
              'client_of_month': 'Cliente del Mes',
              'performance': 'Rendimiento por Desempeño',
              // PlayersTab & BookingModal
              'payment_progress': 'Progreso de cobro',
              'configure_split': 'Configurar División',
              'players': 'Jugadores',
              'court_shared_kiosk': 'Cancha + Kiosco Compartido',
              'recalculate_splits': 'RECALCULAR DIVISIONES',
              'player_details': 'Detalles del Jugador',
              'save_names': 'Guardar Nombres',
              'player_name': 'Nombre del Jugador',
              'enter_name': 'Ingrese nombre...',
              'paid': 'PAGADO',
              'charge': 'COBRAR',
              'shared_rental': 'Alquiler Compartido',
              'individual_extras': 'Extras Individuales',
              'charge_to': 'Cobrar a',
              'select_payment_method': 'Selecciona el método de pago para registrar el cobro.',
              'cash': 'Efectivo',
              'mercadopago': 'Mercado Pago',
              'debit': 'Débito',
              'credit': 'Crédito',
              'error_processing_payment': 'Error al procesar cobro',
              'payment_registered': 'Cobro registrado a',
              'summary_payment': 'Resumen y Pago',
              'kiosk': 'Kiosco',
              'booking_status': 'Estado del Turno',
              'status': 'Estado',
              'free': 'SIN CARGO',
              'completed_status': 'COMPLETADO',
              'pending_status': 'PENDIENTE',
              'total': 'Total',
              'cancel_booking': 'CANCELAR TURNO',
              'close_window': 'CERRAR VENTANA',
              'payment_status': 'Estado de Pago',
              'remaining': 'restantes',
              'no_cost_booking': 'Esta reserva no tiene costo asociado (Gratis).',
              'client_owes': 'El cliente debe abonar el monto restante.',
              'fully_paid': '¡Todo al día! El turno está completamente pagado.',
              'open_match': 'Partido Abierto',
              'visible': 'VISIBLE',
              'hidden': 'OCULTO',
              'level': 'Nivel',
              'gender': 'Género',
              'update_data': 'Actualizar Datos',
              'consumption_details': 'Detalle del Consumo',
              'court_rental': 'Alquiler de Cancha',
              'for': 'Para',
              'general': 'General',
              'minutes': 'Minutos',
              'connection_error': 'Error de conexión',
              'payment_success': 'Pago registrado exitosamente',
              'invalid_amount': 'Ingrese un monto válido',
              'link_copied': 'Link copiado al portapapeles',
              'error_generating_link': 'Error al generar link',
              'confirm_cancel': '¿Estás seguro de que deseas cancelar este turno? Esta acción no se puede deshacer.',
              'booking_cancelled': 'Reserva cancelada exitosamente',
              'error_cancelling': 'Error al cancelar la reserva',
              // Kiosk
              'search_placeholder_kiosk': 'Buscar bebidas, snacks...',
              'assign_consumption': 'Asignar consumo a:',
              'everyone': 'CONSUMEN TODOS',
              'products_available': 'Productos Disponibles',
              'view_all': 'Ver todo',
              'current_consumptions': 'Consumos Actuales',
              'items_count': 'ITEMS',
              'kiosk_total': 'TOTAL KIOSCO',
              // Payment Actions
              'enter_valid_amount': 'Ingrese un monto válido',
              'payment_registered_success': 'Pago registrado exitosamente',
              'register_payment': 'Registrar Cobro',
              'charge_full_amount': 'COBRAR TOTAL',
              'partial_payment': 'Pago Parcial',
              'amount_placeholder': 'Monto',
              'transfer': 'Transferencia',
              'card': 'Tarjeta',
              // Header
              'pending_balance_alert': 'Saldo pendiente',
              'booking_today': 'Turno hoy',
              'booking_expired': 'Turno vencido',
              // Statuses
              'status_PENDING': 'Pendiente',
              'status_CONFIRMED': 'Confirmado',
              'status_PARTIAL_PAID': 'Pago Parcial',
              'status_PAID': 'Pagado',
              'status_IN_PROGRESS': 'En Progreso',
              'status_COMPLETED': 'Completado',
              'status_CANCELED': 'Cancelado',

              // Tournaments - Expanded
              'tournaments_subtitle': 'Gestiona tus campeonatos, visualiza fixtures y sigue el progreso de tus competiciones en tiempo real.',
              'active_tournaments': 'Torneos Activos',
              'registered_teams': 'Equipos Inscritos',
              'upcoming_events': 'Próximos Eventos',
              'all': 'Todos',
              'search_tournament': 'Buscar torneo...',
              'no_results': 'No se encontraron resultados',
              'try_another_search': 'Prueba con otro término de búsqueda.',
              'start_first_tournament': 'Comienza tu Primer Torneo',
              'start_tournament_desc': 'Organiza campeonatos profesionales, gestiona inscripciones, genera fixtures automáticos y lleva el control de resultados en tiempo real.',

              // Create Tournament
              'new_tournament': 'Nuevo Torneo',
              'new_tournament_desc': 'Configura los datos básicos para comenzar.',
              'tournament_name': 'Nombre del Torneo',
              'tournament_name_placeholder': 'Ej. Torneo Apertura 2026',
              'start_date': 'Fecha de Inicio',
              'end_date': 'Fecha de Fin',
              'optional': '(Opcional)',
              'cancel': 'Cancelar',
              'create': 'Crear',
              'back_to_tournaments': 'Volver a Torneos',
              'error_creating_tournament': 'Error al crear torneo',
              'success_creating_tournament': 'Torneo creado exitosamente',

              // Tournament Details
              'public_link': 'Link Público',
              'public_link_copied': 'Link público copiado',
              'configure': 'Configurar',
              'overview': 'Resumen',
              'inscriptions': 'Inscripciones',
              'registered_pairs': 'Parejas Inscritas',
              'scheduled_matches': 'Partidos Programados',
              'tournament_status': 'Estado del Torneo',
              'no_categories': 'No hay categorías configuradas.',
              'go_to_categories': 'Ve a la pestaña "Categorías" para comenzar.',
              'male': 'Masculino',
              'female': 'Femenino',
              'mixed': 'Mixto',
              'enabled_categories': 'Categorías Habilitadas',
              'new_category': 'Nueva Categoría',
              'delete_category_confirm': '¿Estás seguro de eliminar esta categoría? Se borrarán todos los equipos asociados.',
              'category_deleted': 'Categoría eliminada',
              'error_deleting': 'Error al eliminar',
              'register_pair': 'Inscribir Pareja',
              'pair_registered': 'Pareja Inscrita',
              'delete_pair_confirm': '¿Eliminar esta pareja?',
              'pair_deleted': 'Pareja eliminada',
              'no_pairs_in_category': 'No hay parejas inscritas en esta categoría.',
              'generate_fixture': 'Generar Fixture',
              'generate_fixture_desc': 'Crea automáticamente las zonas y partidos para las parejas inscritas.',
              'insufficient_pairs': 'Insuficientes parejas (Mínimo 2)',
              'zones': 'Zonas',
              'generating': 'Generando...',
              'generate': 'Generar',
              'fixture_generated_success': 'Fixture generado exitosamente',
              'reset_fixture': 'Reiniciar Fixture',
              'reset_fixture_confirm': '¿Estás seguro de eliminar TODO el fixture de esta categoría? Se borrarán los partidos.',
              'fixture_active': 'Fixture Activo',
              'group_phase': 'Fase de Grupos',
              'load_result': 'Cargar Resultado',
              'who_won': '¿Quién ganó?',
              'result_sets': 'Resultado (Sets)',
              'result_hint': 'Ingresa el marcador completo de los sets (ej: 6-3 6-4).',
              'save_result': 'Guardar Resultado',
              'result_saved': 'Resultado guardado',
              'select_winner_error': 'Selecciona un ganador',
              'error_select_two_players': 'Debes seleccionar 2 jugadores',
              'level_not_allowed': 'Nivel no permitido',
              'pair_registered_success': 'Pareja inscrita correctamente',
              'error_registering': 'Error al inscribir',
              'player_created_success': 'Jugador creado correctamente',
              'error_creating_player': 'Error al crear jugador',
              'new_player': 'Nuevo Jugador',
              'full_name': 'Nombre Completo',
              'phone': 'Teléfono',
              'category_label': 'Categoría',
              'save_player': 'Guardar Jugador',
              'register_pair_title': 'Inscribir Pareja',
              'player_1': 'Jugador 1',
              'player_2': 'Jugador 2',
              'search_player': 'Buscar Jugador...',
              'create_new_short': 'Nuevo',
              'sum_categories': 'Suma de Categorías',
              'register_pair_action': 'Inscribir Pareja',
              'saving': 'Guardando...',
              'category_created': 'Categoría creada',
              'error_creating_category': 'Error al crear',
              'create_category': 'Crear Categoría',
              'inscription_price': 'Precio Inscripción',
              'name': 'Nombre',
              'tournament_updated': 'Torneo actualizado',
              'error_updating': 'Error al actualizar',
              'tournament_deleted': 'Torneo eliminado',
              'tournament_settings': 'Configuración del Torneo',
              'status_draft': 'Borrador',
              'status_active': 'En Curso',
              'save_changes': 'Guardar Cambios',
              'delete_tournament_title': 'Eliminar Torneo',
              'delete_tournament_confirm': '¿ESTÁS SEGURO? Esto eliminará el torneo, TODAS las categorías, partidos y resultados. No se puede deshacer.',
              'no_phone': 'Sin teléfono',
              'pairs': 'Parejas',
              'bookings': 'Reservas',
              'booking': 'Reserva',
              // Bracket View
              'bracket_view': 'Vista Bracket',
              'list_view': 'Vista Lista',
              'bracket_final': 'Final',
              'bracket_semifinals': 'Semifinales',
              'bracket_quarterfinals': 'Cuartos de Final',
              'bracket_round_of_16': 'Octavos de Final',
              'bracket_round': 'Ronda',
              'bracket_tbd': 'Por definir',
              'bracket_champion': 'Campeón',
              'bracket_no_elimination': 'No hay partidos de eliminación para mostrar en el bracket.',
              'bracket_zoom_in': 'Acercar',
              'bracket_zoom_out': 'Alejar',
              // New Tournament/League Logic
              'tournament_type': 'Tipo de Competencia',
              'tournament_only': 'Torneo',
              'league_only': 'Liga',
              'points_victory': 'Pts. Victoria',
              'points_draw': 'Pts. Empate',
              'points_loss': 'Pts. Derrota',
              'points_loss_noshow': 'Pts. No Presentación',
              'points_walkover': 'Pts. Walkover',
              'format_direct': 'Eliminación Directa',
              'format_groups': 'Grupos + Eliminatorias',
              'format_round_robin': 'Round Robin (Todos contra todos)',
              'format_league': 'Liga por Jornadas',
              'match_type': 'Modalidad',
              'individual': 'Individual',
              'pairs_mode': 'Parejas',
              'standing_table': 'Tabla de Posiciones',
              'fixture': 'Fixture',
              'matchdays': 'Jornadas',
              'matchday': 'Fecha',
              'groups_zones': 'Grupos / Zonas',
              'knockout': 'Cruces / Playoff',
              'points_short': 'PTS',
              'played_short': 'PJ',
              'won_short': 'PG',
              'lost_short': 'PP',
              'draw_short': 'PE',
              'sets_for_short': 'SF',
              'sets_against_short': 'SC',
              'games_for_short': 'GF',
              'games_against_short': 'GC',
              'diff_short': 'DIF',
              'rank': 'Pos',
              'generate_fixture_league': 'Generar Fixture de Liga',
              'generate_fixture_elimination': 'Generar Cuadro Directo',
              'generate_fixture_groups': 'Generar Fase de Grupos'
       },
       en: {
              'dashboard': 'Panel',
              'card': 'Card',
              // Header
              'pending_balance_alert': 'Pending Balance',
              'booking_today': 'Booking Today',
              'booking_expired': 'Booking Expired',
              // Statuses
              'status_PENDING': 'Pending',
              'status_CONFIRMED': 'Confirmed',
              'status_PARTIAL_PAID': 'Partially Paid',
              'status_PAID': 'Paid',
              'status_IN_PROGRESS': 'In Progress',
              'status_COMPLETED': 'Completed',
              'status_CANCELED': 'Cancelled',
              // Bracket View
              'bracket_view': 'Bracket View',
              'list_view': 'List View',
              'bracket_final': 'Final',
              'bracket_semifinals': 'Semifinals',
              'bracket_quarterfinals': 'Quarterfinals',
              'bracket_round_of_16': 'Round of 16',
              'bracket_round': 'Round',
              'bracket_tbd': 'TBD',
              'bracket_champion': 'Champion',
              'bracket_no_elimination': 'No elimination matches to display in bracket view.',
              'bracket_zoom_in': 'Zoom In',
              'bracket_zoom_out': 'Zoom Out',
              'tournament_type': 'Competition Type',
              'tournament_only': 'Tournament',
              'league_only': 'League',
              'points_victory': 'Pts. Victory',
              'points_draw': 'Pts. Draw',
              'points_loss': 'Pts. Loss',
              'points_loss_noshow': 'Pts. No Show',
              'points_walkover': 'Pts. Walkover',
              'format_direct': 'Direct Elimination',
              'format_groups': 'Groups + Knockout',
              'format_round_robin': 'Round Robin',
              'format_league': 'League Matchdays',
              'match_type': 'Modality',
              'individual': 'Individual',
              'pairs_mode': 'Pairs',
              'standing_table': 'Standings',
              'fixture': 'Fixture',
              'matchdays': 'Matchdays',
              'matchday': 'Matchday',
              'groups_zones': 'Groups / Zones',
              'knockout': 'Knockout / Playoff',
              'points_short': 'PTS',
              'played_short': 'P',
              'won_short': 'W',
              'lost_short': 'L',
              'draw_short': 'D',
              'sets_for_short': 'SF',
              'sets_against_short': 'SA',
              'games_for_short': 'GF',
              'games_against_short': 'GA',
              'diff_short': 'DIF',
              'rank': 'Pos',
              'generate_fixture_league': 'Generate League Fixture',
              'generate_fixture_elimination': 'Generate Bracket',
              'generate_fixture_groups': 'Generate Group Phase'
       }
}

interface LanguageContextType {
       language: Language
       setLanguage: (_lang: Language) => void
       t: (_key: string) => string
}

const LanguageContext = createContext<LanguageContextType | undefined>(undefined)

export function LanguageProvider({ children }: { children: React.ReactNode }) {
       const [language, setLanguage] = useState<Language>('es')

       useEffect(() => {
              const saved = localStorage.getItem('language') as Language
              if (saved) setLanguage(saved)
       }, [])

       const handleSetLanguage = (lang: Language) => {
              setLanguage(lang)
              localStorage.setItem('language', lang)
       }

       const t = (key: string) => {
              return translations[language][key] || key
       }

       return (
              <LanguageContext.Provider value={{ language, setLanguage: handleSetLanguage, t }}>
                     {children}
              </LanguageContext.Provider>
       )
}

export const useLanguage = () => {
       const context = useContext(LanguageContext)
       if (!context) throw new Error('useLanguage must be used within a LanguageProvider')
       return context
}
