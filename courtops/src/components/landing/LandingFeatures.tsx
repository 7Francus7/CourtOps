...
export default function LandingFeatures() {
       ...
       return (
              <section className="py-24 px-6 bg-slate-50 relative overflow-hidden" id="features">
                     {/* Background elements */}
                     <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white via-slate-50 to-slate-50 opacity-80" />

                     <div className="max-w-7xl mx-auto relative z-10">
                            <div className="text-center max-w-3xl mx-auto mb-20 space-y-6">
                                   <span className="text-emerald-600 font-bold uppercase tracking-widest text-xs bg-emerald-100/50 px-3 py-1 rounded-full border border-emerald-200">Características Potentes</span>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                                   {FEATURES.map((feature, idx) => (
                                          <motion.div
                                                 key={idx}
                                                 whileHover={{ y: -5 }}
                                                 className="group relative p-8 rounded-[2rem] bg-white border border-slate-200 hover:border-emerald-200 hover:shadow-xl hover:shadow-emerald-500/10 transition-all duration-300"
                                          >
                                                 ...
                                                 {/* Hover corner effect */}
                                                 <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-slate-100 to-transparent rounded-tr-[2rem] rounded-bl-[4rem] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none`} />
                                          </motion.div>
                                   ))}
                            </div>
                     </div>
