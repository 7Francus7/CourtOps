...
export async function GET(request: Request) {
  try {
    ...
    if (!startStr || !endStr) {
      return new NextResponse('Parametros start y end requeridos', { status: 400 })
    }

    const parsedRange = parseDateRange(startStr, endStr)
    if (!parsedRange) {
      return new NextResponse('Rango de fechas invalido', { status: 400 })
    }
    ...
    appendSummaryRow(summarySheet, 'Periodo', dateRangeLabel, 'Rango completo incluido en la exportacion')
    ...
    const dailySheet = workbook.addWorksheet('Evolucion diaria')
    setupWorksheet(
      dailySheet,
      `Evolucion diaria - ${clubName}`,
      `Periodo ${dateRangeLabel}`,
      [
        { header: 'Fecha', key: 'date', width: 14 },
    ...
    setupWorksheet(
      ...
      `Periodo ${dateRangeLabel}`,
      [
        { header: 'Metodo', key: 'method', width: 24 },
        { header: 'Ingresos', key: 'income', width: 16 },
        { header: 'Egresos', key: 'expense', width: 16 },
        { header: 'Neto', key: 'net', width: 16 },
    ...
    setupWorksheet(
      categoriesSheet,
      `Categorias financieras - ${clubName}`,
      `Periodo ${dateRangeLabel}`,
      [
    ...
    const occupancySheet = workbook.addWorksheet('Ocupacion')
    setupWorksheet(
      occupancySheet,
      `Ocupacion por cancha - ${clubName}`,
      `Periodo ${dateRangeLabel}`,
      [
        ...
        { header: 'Ocupacion', key: 'occupancy', width: 14 },
      ],
    )
